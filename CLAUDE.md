...
- Dynamic node positioning based on system groups (per-repo, defined in `demoData.ts`)
...
**Data Architecture**: Graph data lives in `src/data/demoData*.ts` files, one per repository:
- `demoData.ts` — PostHog (types + shared helpers like `getNodesForRepo()`, `getSystemGroupsForRepo()`)
- `demoDataDaytona.ts`, `demoDataDaytonaScip.ts`, `demoDataDaytonaComplete.ts` — Daytona variants
- `demoDataKsync.ts` — ksync
- `GraphNode` and `GraphEdge` types define the structure; nodes are categorized by type and grouped into `SystemGroup`s for layout
- Edges represent relationship types (import, export, call, data)
- `SystemGroup` IDs are prefixed by repo: PostHog uses descriptive names (`web-app`, `plugin-server`), Daytona uses `d-*`, ksync uses `k-*`
...
Tests use Vitest (configured separately in `vitest.config.ts`) with jsdom environment:
